...
- [ ] ES 에서 BigQuery 로 Batch 처리 시 중복되는 데이터 문제 알고리즘으로 해결..? 방법 찾기. (2023 - 10 - 07)  
      ㄴ 기존에는 n 개의 데이터를 검색 후 날짜를 기준으로 배치 처리하는데, 데이터가 많아지면 느려짐. TC = O(n) __AWS S3__ 를 이용하면 될 것 같긴 함.
